(function($) {
	...
	tagBox = {
		...
		parseTags : function(el) {
			var id = el.id, num = id.split('-check-num-')[1], taxbox = $(el).closest('.neuwiki-tags'),
		...
		init : function() {
			...
			$('.neuwiki-tags').each( function() {
				tagBox.quickClicks(this);
			});

			$('input.tagadd', ajaxtag).click(function(){
				t.flushTags( $(this).closest('.neuwiki-tags') );
			});
			...
			$('input.newtag', ajaxtag).blur(function() {
				...
				if ( 13 == e.which ) {
					tagBox.flushTags( $(this).closest('.neuwiki-tags') );
					return false;
				}
			}).keypress(function(e){
				if ( 13 == e.which ) {
				...
				var tax = $(this).closest('div.neuwiki-tags').attr('id');
			...
			$('#post').submit(function(){
				$('div.neuwiki-tags').each( function() {
